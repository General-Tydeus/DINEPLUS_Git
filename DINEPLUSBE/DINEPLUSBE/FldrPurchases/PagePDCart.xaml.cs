    ...
    public partial class PagePDCart : ContentPage
    {
        ...
        private async void BtnSave_Clicked(object sender, EventArgs e)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var result = await client.PostAsync($"{new ClsGetIPAddress().GetIPAddress()}/API/WEBAPI/Voucher/InsertMain1", content);
                if (result.IsSuccessStatusCode)
                {
                    var clientGet = new HttpClient();
                    clientGet.BaseAddress = new Uri($"{new ClsGetIPAddress().GetIPAddress()}/API/WEBAPI/Various/GetDoorMessage/?strWAPIVoucher=CV");
        ...
        public ModeltblMain1 tblSavetblMain1()
        {
            return new ModeltblMain1()
            {
                ...
                UserCode = "a",
                TDate = DateTime.Now,
                //Reference = PagePDProductSearchList.Instance.t,
                ControlNo = "a",
                Remarks = "a",
                Void = true,
                CNCode = "a",
